package simpleml.utils;

import simpleml.classify.ext.LoadException;
import simpleml.struct.*;
...
public class VectorUtils {
  ...
  public static MutableVector newSparseVector(List<IndexedValue> values, int dimension) {
    try {
      return (MutableVector) DEFAULT_SPARSE_VECTOR_CLASS.getConstructor(Collection.class, int.class).newInstance(values, dimension);
    } catch (NoSuchMethodException | InstantiationException | IllegalAccessException | InvocationTargetException e) {
      throw new RuntimeException(e);
    }
  }
